...
namespace SmartSpawn {
	...
	public class SmartSpawnScript : MonoBehaviour {
		...
		protected virtual void SpawnItem(Vector3 spawnLocation, int index) {
			...
			++spawnCount;
			if (spawnCount % 5 == 0) _gameObject = goldenFish as GameObject;
			else _gameObject = itemPrefabs[index] as GameObject; ;

			if (_gameObject != null){

				GameObject newItem = Instantiate(_gameObject, spawnLocation, _gameObject.transform.rotation) as GameObject;
				//GameObject newItem = SimplePool.Spawn(_gameObject, spawnLocation, _gameObject.transform.rotation) as GameObject;
